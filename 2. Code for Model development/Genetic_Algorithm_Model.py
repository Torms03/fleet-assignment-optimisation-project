...
# The GA model is adapted from Marlesson. (2020). Genetic Algorithm with DEAP. Available at: https://www.kaggle.com/code/marlesson/genetic-algorithm-with-deap
## Create the Fitness and Individual classes using the DEAP creator
# Define a 'FitnessMin' class that minimises the cost (negative weights mean minimising)
creator.create("FitnessMin", base.Fitness, weights=(-1.0,))

# Define an 'Individual' class as a list of aircraft types
creator.create("Individual", list, fitness=creator.FitnessMin)

# Initialise an individual (random aircraft assignments)
def init_individual():
...
# Cost Function (the goal is to minimise the total cost)
def eval_cost(individual):
    total_cost = 0  # Initialise the total cost to 0
    fleet_used = {}  # Create an empty dictionary to track how many of each aircraft type are used
    
    # Initialise the fleet_used dictionary with all aircraft types set to 0
    ...
    if aircraft_range.get(ac, 0) < distance:
        total_cost += 100000  # Penalty for range violation
...
for i, ac_type in enumerate(best_ind):
    ...
    if aircraft_range.get(ac_type, 0) < distance:
        ...
        total_range_penalty += 100000  # Fixed penalty per violation 
