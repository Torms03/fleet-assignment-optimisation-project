...
# The ACO model is adapted from Akaval. (2020). AntColonyOptimization. Available at: https://github.com/Akavall/AntColonyOptimization/blob/master/ant_colony.py 
# Initialise pheromone levels for Ant Colony Optimisation
pheromone = {}
for ectrl_id in uk_flights_data['ECTRL_ID'].unique():
    pheromone[ectrl_id] = {ac_type: 1.0 for ac_type in aircraft_types}

# Hyperparameters for Ant Colony Optimisation (ACO)
n_ants = 30  # Number of ants (solutions) to explore
n_iterations = 150  # Number of iterations (loops) for the ACO algorithm
alpha = 1  # Influence of pheromone level on decision-making
beta = 2  # Influence of cost on decision-making
...
for iteration in range(n_iterations):  # Loop through each iteration
    ...
    for ant in range(n_ants):  # Loop through each ant
        ...
        total_cost = 0  # Initialise the total cost for this solution
        ...
        # Filter aircraft that are available and have the required range
            ...
            if not available_aircraft:
                ...
                # The prob equation is derived from Yilmaz Eroglu, D. and Akcan, U. (2024). An adapted ant colony optimization for feature selection. Applied Artificial Intelligence, 38(1), p.2335098. Available at: https://doi.org/10.1080/08839514.2024.2335098 
                for ac in available_aircraft:
