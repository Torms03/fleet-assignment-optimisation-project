...
# Load September 2019 Eurocontrol Flights dataset
...
# The country code reference is derived from Avcodes (n.d.). ICAO Nationality Codes. Available at: https://www.avcodes.co.uk/icaonat.asp
uk_flights_data = flights_data[(flights_data['ADEP'].str[:2] == 'EG') | (flights_data['ADES'].str[:2] == 'EG')]

# Filter for flights by British Airways, BA Cityflyer and BA Euroflyer (BAW, CFE, EFW)
# The British Airways code is derived from IATA (n.d.) British Airways. Avaialable at: https://www.iata.org/en/about/members/airline-list/british-airways/52/
# The BA Cityflyer code is derived from CAPA (n.d.) BA Cityflyer. Avaialable at: https://centreforaviation.com/data/profiles/airlines/ba-cityflyer-cj
# The BA Euroflyer code is derived from CAPA (n.d.) BA Euroflyer. Avaialable at: https://centreforaviation.com/data/profiles/airlines/ba-euroflyer-a0
...
# The codes for London airports are derived from TFL. (n.d.). London airports and rail connections. Avaialable at: https://tfl.gov.uk/travel-information/visiting-london/getting-to-london/london-airports
...
# The BA fleet list is derived from IAG. (n.d.). British Airways Report and Accounts year ended 31 December 2019. Available at: https://www.iairgroup.com/investors-and-shareholders/financial-reporting/annual-reports/
BA_fleet_AC_type = ['A318', 'A319', 'A320', 'A321', 'A339', 'A35K', 'A388', 'B744', 'B772', 'B773', 'B788', 'B789', 'SB20', 'E170', 'E190']

# Replace aircraft type codes for consistency with BA fleet types
...
# The BA fleet size is derived from IAG. (n.d.). British Airways Report and Accounts year ended 31 December 2019. Available at: https://www.iairgroup.com/investors-and-shareholders/financial-reporting/annual-reports/
...
# The cost per aircraft type is derived from Eurocontrol. (2024). EUROCONTROL Standard Inputs for Economic Analyses. Available at : https://ansperformance.eu/economics/cba/standard-inputs/
...
# The sources for aircraft capacity is provided under in Appendix A3
...
# Load 2019 September OAG passenger traffic data 
passenger_data = pd.read_csv("D:\FYP\FINAL YEAR PROJECT PROGRESS\Datasets\gb_pax_traffic.csv")

# Load file for airport codes mapping
# This data is sourced from Airport Codes. (n.d.). IATA Airport Codes. Available on: https://airportcodes.aero/iata/Z
airport_codes = pd.read_excel("D:\FYP\FINAL YEAR PROJECT PROGRESS\Datasets\Airport codes IATA-ICAO.xlsx")

# Map IATA codes to ICAO codes for both origin and destination airports
...
# Further filter for non-stop itineraries (direct flights)
...
# Function to get the demand for a specific route (arbitrary value of 50 if not found)
...
# The haversine formula is adapted from Rocha, B. (2012). haversine. Available at: https://gist.github.com/rochacbruno/2883505
...
# Define aircraft range. Note that A318 and A339 were in nm and converted to km.
# The sources for aircraft range is provided under in Appendix A3
aircraft_range_raw = {
...
# The ILP model is adapted from PuLP. (n.d.a). A Transportation Problem. Available at: https://coin-or.github.io/pulp/CaseStudies/a_transportation_problem.html
# Define the linear programming problem as a minimisation problem
...
# Objective function: Minimise cost and penalties for constraint violations
problem += (
    ...
    + 1e5 * lpSum(slack_demand.values())          # Penalty for unmet demand
...
# The run time calculation is derived from GeeksforGeeks. (2023). How to check the execution time of Python script? Available at: https://www.geeksforgeeks.org/how-to-check-the-execution-time-of-python-script/
start_time = time.time()  # Start timing the optimisation process
...
# Total cost of actual assignment
print(f"Total cost of actual assignment: £{flights_filtered['cost'].sum()}")
...
total_assignment_penalty = sum(slack_var.value() for slack_var in slack_one_aircraft.values()) * 1e5
